...
### Benefits of Analysis
  As previous research has already suggested that being exposed to a variety of teammates over the course of a career can have benefitial impacts on offensive metrics, this analysis should evaluate if there is a similar relationship between a player and his coaches.  This study could demonstrate the possible benefits of trading young players in the hopes that exposing them to a plethora of coaching styles which could lead to greater player development.
...
## Data Exploration
  ...
  As the experience and insight that is to be gained from coaches should last for a player's full career, all of the offensive metrics from the `Lahman` database needs to be combined from yearly observations to an observation that spans the full duration of a player's performance from `r min(battingStatistics$yearID)`-`r max(battingStatistics$yearID)`.  In addition to this transformation, the `Lahman` database does not contain the Batting Average or Slugging statistics, so those variables had to be calculated.
